...
<div class="card">
    ...
    <div class="card-body">
        ...
        <a href="{{route('buku.create')}}"><button type="submit" class="btn btn-
primary"><i class="fa fa-plus"></i> Tambah</button></a>
        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $data->nama}}</td>
                    <td><img src="{{Storage::url('buku/'.$data->foto)}}" style="width:150px" class="img-thumbnail">

                    </td>
                    <td>{{ $data->penerbit}}</td>
                    <td>{{ $data->pengarang}}</td>
                    <td>
                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('buku.destroy', $data->id) }}" method="POST">
                        ...
                        <a href="{{ route('buku.edit', $data->id) }}" class="btn btn-
...
<script src="{{ url('plugins/datatables-responsive/js/responsive.bootstrap4.min.j')
}}s"></script>
